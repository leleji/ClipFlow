...
namespace ClipFlow.Desktop.MacOS.Services
{
    public class MacOsClipboardService : IClipboardHandler
    {
        ...
        public extern static bool AXIsProcessTrusted();
        ...
        public async Task<ClipboardData?> GetContentAsync()
        {
            ...
            try
            {
                ...
                else if (types.Contains(ClipboardFormat.MacText))
                {
                ...
                else if (types.Contains(ClipboardFormat.MacImageJpeg) || types.Contains(ClipboardFormat.MacImagePng) ||
            ...
            try
            {
                ...
                switch (data.Type)
                {
                    case ClipboardType.Text:
                        pasteboard.SetStringForType(data.Text, ClipboardFormat.Text);
                    ...
                    case ClipboardType.FileList:
                        if (data.CopyFiles.Count > 0)
                        {
                            ...
                            INSPasteboardWriting[] fileUrls = data.CopyFiles.Cast<string>()
